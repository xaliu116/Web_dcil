...
class PageDecoder(object):
    ...
    def find_next(self, cors, pred_read, conf):
        n_samples = len(cors)
        unfound = np.ones(n_samples).astype(bool)
        uncycle = np.ones(n_samples).astype(bool)
        reach_border = np.zeros(n_samples).astype(bool)
        next_indies = np.ones(n_samples).astype(np.int32) * -1
        paths = np.ones((self.max_steps+1, n_samples, 2)).astype(int) * -1
        paths[0, :, :] = cors 
